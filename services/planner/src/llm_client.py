...
# - 재활용: 목적은 **재질별 분리**이므로 이름의 재질과 지시의 목적지를 매칭한다(음식물은
#   재질이 아니라 food_waste라는 별도 카테고리로 다룬다).
...
- **food_waste(음식물쓰레기)는 재질이 아니라 별도 카테고리다.** 지시가 '음식물'/
  '음식물쓰레기'를 가리키면 재질 매칭과 같은 방식으로 food_waste 물체를 그 목적지로 보낸다.
