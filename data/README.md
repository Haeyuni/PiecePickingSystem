...
- `samples/`: 학습·평가용 샘플 데이터. `perception_capture.py`로 뜬 프레임을 여기에 둔다.
  **예외로 `vlm_sam_reference.png` 하나만 git으로 관리한다** — `docs/vlm_sam_pipeline.md`의
  실측 수치가 전부 이 사진 기준이라, 사진이 없으면 그 결과를 재현할 수 없다.
