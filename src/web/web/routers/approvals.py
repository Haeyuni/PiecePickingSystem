...
class ApprovalRequest(BaseModel):
    ...
    action: str          # "approve" | "reject" | "correct_label" | "correct_bin" | "exclude_object"
...
def submit_approval(trace_id: str, body: ApprovalRequest):
    if body.action not in (
        "approve", "reject", "correct_label", "correct_bin", "exclude_object",
    ):
        return JSONResponse(
            status_code=400,
    ...
    if body.action == "exclude_object" and not body.object_id:
        return JSONResponse(
            status_code=400,
            content={"schema_version": "1.0.0",
                     "error": {"code": "MISSING_OBJECT_ID",
                               "message": "exclude_object는 object_id가 필요합니다"}},
        )
