...
def selection(node, candidate_msg, age_s=0.0, observation_id="obs-1"):
    node._selected_grasp = (time.monotonic() - age_s, observation_id, candidate_msg)


def test_no_selection_highlights_nothing(node):
    """첫 pick 전에는 고른 것이 없다 — 아무것도 초록으로 칠하지 않는다."""
    assert node._selected_index([candidate("obj_001#0"), candidate("obj_001#1")], "obs-1") is None
...
def test_selected_candidate_is_found_by_pose(node):
    ...
    assert node._selected_index(candidates, "obs-1") == 1
...
def test_far_pose_does_not_match(node):
    ...
    assert node._selected_index(candidates, "obs-1") is None
...
def test_empty_candidate_list_is_safe(node):
    ...
    assert node._selected_index([], "obs-1") is None


def test_other_observation_selection_is_ignored(node):
    candidates = [candidate("obj_001#0")]
    selection(node, candidate("obj_001#0"), observation_id="obs-old")
    assert node._selected_index(candidates, "obs-current") is None
