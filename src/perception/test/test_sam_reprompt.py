...
def previous(class_name="tape", position=(0.0, 0.0, 500.0), extent=None, **extra):
    ...
    return {"class_name": class_name, "name_ko": "테이프", "is_new_class": False,
            "confidence": 0.9, "position_base_mm": position,
            "extent_base_mm": extent or [], **extra}
...
def test_labels_are_inherited_not_recomputed():
    ...
    detector = make()
    detector.prime([previous(class_name="toothpaste", is_new_class=True)],
                   IDENTITY, IDENTITY, INTRINSICS)

    assert detector.expected[0]["class_name"] == "toothpaste"
    assert detector.expected[0]["is_new_class"] is True
