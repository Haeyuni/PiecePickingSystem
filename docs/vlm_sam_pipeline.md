...
- 신규 클래스 등록(FR-05a~c)과 이어지지 않았다. VLM이 낸 속성은 `needs_confirmation=true`로
  발행될 뿐 기록되지 않는다 — 매 관측마다 다시 VLM 추정값을 낸다. 이 경로는 미완성인 채로
  남겨졌다(planner `vlm_client.py` TODO) — DB `object_attributes`로 확인값을 기록하고
  우선 쓰는 경로를 만드는 대신, 2026-09-09 그 테이블과 확인 UI를 통째로 제거했다
  (시스템명세서 3.1a절). vlm_sam 경로는 애초에 이 테이블을 보지 않았으므로 이 한계는
  그대로 남는다.
