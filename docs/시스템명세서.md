...
`execution_logs.object_id`는 런타임 인스턴스 식별자(`obj_003`)이고, `class_name`은 실행 당시 분류 결과를 보존하는 비정규화 스냅샷이다. `class_name`에는 `object_attributes` FK를 두지 않는다. 따라서 속성 마스터에 아직 없는 신규 클래스도 실행 로그를 남길 수 있고, 향후 속성 테이블을 제거해도 과거 실행 이력은 유지된다.
