...
5. **확인 전까지 해당 클래스의 모든 파지 시도는 `fallback`(grip_level 5, 가장 약하게)을 강제로 사용한다** — VLM이 제안한 값은 확인 전에는 제어 파라미터에 반영되지 않는다(NFR-03a).
...
`object_attributes`에 사람이 확인한 handler profile을 저장하고, planner의 결정적 검증기가
그 값을 계획에 전달하며, control이 시점별 최대 안전 힘의 **범위 안에서만** 힘을 올린다.
신규 클래스에 대해서만 VLM이 속성과 단계를 **제안**할 수 있고, 사용자 확인 전에는 위
5번의 `fallback`(5=가장 약하게)을 유지한다. 실행 결과만으로 DB의 힘을 자동 상향하지 않는다.
