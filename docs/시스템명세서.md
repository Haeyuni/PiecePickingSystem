...
**향후 확장 — 물체별 파지력 정책**: 현재 `normal`/`fragile`/`deformable` 세 프로파일은
초기 안전 기본값이다. 그러나 같은 `deformable`이어도 물티슈는 눌림을 허용하면서 미끄럼을
막을 힘이 필요하고, 치약·액체 파우치는 과도하게 누르면 누출되거나 터질 수 있으며, 인형은
더 큰 압축을 허용할 수 있다. 따라서 실물 데이터가 쌓이면 `compressible_slippery`,
`liquid_soft`, `compressible_safe`처럼 **취급 특성과 안전 상한이 다른 handling profile**로
세분화한다.

이 확장에서 LLM은 profile이나 힘(N)을 실행 시점에 직접 결정하지 않는다. 알려진 클래스는
`object_attributes`에 사람이 확인한 handling profile을 저장하고, planner의 결정적 검증기가
그 값을 계획에 전달하며, control이 profile을 실제 파지력·닫기 목표·접근 속도로 변환한다.
신규 클래스에 대해서만 VLM이 `fragile`/`liquid-filled`/`compressible`/`slippery` 같은 속성과
profile을 **제안**할 수 있고, 사용자 확인 전에는 위 5번의 `fragile` fallback을 유지한다.
실행 결과만으로 DB의 힘을 자동 상향하지 않는다.

단계적 파지력 제어를 도입할 때도 profile별 최대 안전 힘 안에서만 힘을 올린다. 그리퍼의
실제 정지 폭·Grip detected와 들어올린 뒤 폭 변화 또는 시각 검증을 함께 기록해 미끄러짐을
판정하고, 검증된 결과를 다음 profile 튜닝 근거로 사용한다. 2026-09-08 물티슈 실측에서는
클라우드 폭 72.7mm에 닫기 목표를 43.6mm로 주었지만 12N에서 실제 폭 70.5mm로 멈췄다.
목표 폭은 이미 충분히 좁으므로 close ratio를 더 낮추기보다 물티슈 전용 profile의 허용 힘을
검증하는 사례다. 공용 `deformable` 힘을 올리면 치약에도 적용되므로 물체 특성 분리 전에
전역값을 올리지 않는다.
