...
`execution_logs.grasp_strategy`는 `sort_msgs/GraspCandidate.msg`의 `strategy` 상수(`heuristic_pca` / `contact_graspnet` / `graspnet_baseline`)와 **동일한 문자열**을 써야 한다. 메시지 상수와 DB 기록값의 표기가 어긋나면 조인 쿼리가 깨진다. `db/migrations/001_init.sql`에서 CHECK 제약으로 강제한다 — 표기 규칙을 문서로만 두면 지켜지지 않는다.
...
SELECT grasp_strategy,
       COUNT(*) FILTER (WHERE result = 'success') * 100.0 / COUNT(*) AS success_rate
FROM execution_logs
WHERE skill_name = 'pick'
GROUP BY grasp_strategy
ORDER BY success_rate DESC;
```

**2026-09-10에 `experiment_runs`·`calibration_samples` 테이블과 `execution_logs.experiment_tag` 컬럼을 제거했다**(`010_drop_unused_experiment_calibration.sql`) — `object_attributes`(2.1절)와 같은 패턴으로, 값을 실제로 채우는 코드가 저장소 어디에도 없었다. `experiment_runs`는 INSERT/SELECT 코드가 없었고(실험 조건 비교는 `execution_logs.grasp_strategy` 자체 컬럼으로 이미 가능했다), `calibration_samples`는 `tools/calibration/handeye_calibration.py`가 DB 대신 `tools/calibration/data/calibrate_data.json` 파일에만 결과를 남겨서 처음부터 이 테이블을 보지 않았다.
...
| 실험 설정 관리 | `docker-compose.yml` 환경변수(예: `GRASP_STRATEGY`) + YAML config. 별도 DB 태깅 테이블(`experiment_runs`)은 2026-09-10 제거 — `execution_logs.grasp_strategy` 자체 컬럼으로 비교 가능 |
...
| NFR-09 (실험 재현성) | 2.3절 `eval_runs`/`eval_results` 테이블, `execution_logs.grasp_strategy` |
