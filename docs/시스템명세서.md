...
BR 문서(v2)와 인터페이스 정의서(`인터페이스_정의서.md`)의 요구사항을 구현 가능한 수준으로 구체화한 기술 명세서다. 저장소 `PiecePickingSystem`(컨테이너 안에서는 `/ros2_ws`)의 실제 패키지 구성을 기준으로 작성했다 — 배치는 5.3절, 실행 순서는 저장소 `README.md` 6절을 본다.
...
|---|---|---|---|
| 0 | 1차 | 캘리브레이션 | `tools/calibration/` (오프라인 스크립트, 컨테이너 밖) |
| 1 | 1차 | 검출 확인 | perception |
| 2 | 1차 | 세그멘테이션 | perception |
| 3 | 1차 | 파지 방법 비교 (3전략 계획 → 2전략 구현, 3.2절) | grasp |
| 4 | 1차 | LLM 지시 기반 분류 (**1차 목표 달성 지점**) | planner, web, db |
| 5 | 2차 | 신규 품목 대응 — **클래스 단위 확인 루프는 미구현으로 제거**(3.1a절), 대신 명령 1건당 실행 승인(라벨 수정 포함)으로 대체 | planner, web |
| 6 | 2차 | 음성 인터페이스(STT + 웨이크워드) | web(`stt.py`), `tools/voice/wakeword_bridge.py` |
...
```
[웹 UI] --- [로봇 제어박스] --- [M0609 + OnRobot RG2 + 손목 RealSense]
    |
[GPU 워크스테이션 = 로봇 PC]
    ├─ perception  (온디맨드 관측, world_state_raw/instance_masks 발행)
    ├─ grasp       (파지 자세 추정, world_state 최종 발행)
    ├─ graspnet    (GraspNet-baseline 상주 추론 서버, grasp가 HTTP로 부른다)
    ├─ planner     (LLM/VLM 플래닝 + 검증기)
    ├─ control     (ROS2 스킬 실행기, 순응제어)
    ├─ web         (FastAPI + rclpy 브리지 + 프론트 정적 자산)
    └─ db          (PostgreSQL)
```

6개 애플리케이션 컨테이너 + 1개 DB 컨테이너로 구성한다. 각 컨테이너는 독립적으로 재빌드·재배포 가능해야 한다(NFR-06). `graspnet`은 `grasp`가 `graspnet_baseline` 전략으로 돌 때만 필요하다 — 없어도 `grasp`는 뜨고, 그 관측만 후보 없이 건너뛴다.
...
erDiagram
    ...
    EXECUTION_LOGS {
        ...
        int retry_count
        float cycle_time_ms
        text observation_image_path
        text point_cloud_path
...
|---|---|---|---|
| `src/perception/` | Python 3.12 (ROS2 Jazzy, ament_python) | ultralytics(YOLO11-seg, SAM2), OpenCV, NumPy, rclpy, httpx | GPU 필요. `detectors/`에 `yolo`·`vlm_sam` 두 경로 |
| `src/grasp/` | Python 3.12 (ROS2 Jazzy, ament_python) | NumPy(포인트클라우드·PCA), httpx로 `graspnet` 서버 호출 | GPU는 추론 서버 쪽이 쓴다. `strategies/`에 전략 파일 |
| `src/control/` | Python 3.12 (ROS2 Jazzy, ament_python) | rclpy, doosan-robot2(`dsr_msgs2`), `onrobot_rg_msgs` | GPU 불필요, 실시간성 요구. MoveIt2는 쓰지 않는다 — `MovejH2r`/`MovelH2r` 액션으로 직접 움직인다 |
| `src/web/` | Python 3.12 (ROS2 Jazzy, ament_python) + TypeScript | FastAPI(`main.py`), rclpy(`ros_bridge.py`로 격리), faster-whisper(`stt.py`) | `main.py`·`routers/`는 ROS2를 모르는 순수 HTTP 계층으로 유지 |
| `services/planner/` | Python 3.12 (**COLCON_IGNORE**, 순수 Python) | FastAPI(`app.py`), OpenAI SDK(`llm_client.py`/`vlm_detect.py`, 개발계획 D-3), Pydantic(`schema.py`), DB 클라이언트(`db.py`) | GPU 불필요. ROS2와 무관 — web을 거쳐서만 control에 도달 |
| `src/grasp/docker/graspnet_baseline/` | Python (별도 이미지) | GraspNet-baseline + PyTorch/CUDA | 상주 HTTP 추론 서버(`:8200`). ROS2 없음 |
...
모델은 `faster-whisper`(CTranslate2 기반) `small`/`base`를 CPU에서 INT8 양자화로 구동하는 것을 기본값으로 한다. 처리 흐름: 브라우저 마이크 입력 → `POST /api/stt`로 오디오 blob 전송 → `stt.py`가 텍스트 변환 → 변환된 텍스트를 기존 명령 입력 경로에 그대로 주입(FR-23 — STT 전용 처리 경로를 별도로 두지 않음).

**웨이크워드는 브라우저가 아니라 로봇 PC에서 듣는다**(2026-09-09). 브라우저 SpeechRecognition(Web Speech API)은 사전에 없는 단어("rokey")를 신뢰성 있게 못 잡는 것이 실측으로 확인돼, 로봇 PC의 마이크를 openwakeword 커스텀 모델로 직접 듣는 `tools/voice/wakeword_bridge.py`로 대체했다. 감지되면 `POST /api/internal/wake-detected` → `web`이 `/ws/live`로 중계 → 열려 있는 브라우저 탭이 녹음을 시작한다. **녹음과 STT 자체는 여전히 브라우저 마이크 → `/api/stt` 경로다** — 웨이크워드만 옮겨왔다. 브리지는 드라이버처럼 사용자가 직접 띄우고 끄며, 참조하는 모델(`corecode/VoiceProcessing`)은 이 저장소가 관리하지 않는 로봇 PC 로컬 자산이다.
...
**구현 현황 (2026-09-10)**: `heuristic_pca`와 `graspnet_baseline` 두 개가 `strategies/__init__.py` 레지스트리에 등록돼 있고, **기본값은 `graspnet_baseline`**이다(`docker-compose.yml`의 `GRASP_STRATEGY`). `contact_graspnet`은 가중치와 별도 CUDA 환경이 준비되기 전까지 등록하지 않는다 — 미등록 이름을 요청하면 heuristic으로 조용히 대체되지 않고 예외가 난다(`strategies.get`). 전략 비교 실험 결과가 실수로 1개 전략 값으로 뒤섞이는 것을 막기 위해서다.

`graspnet_baseline`은 노드 안에서 모델을 올리지 않고 **상주 추론 서버**(`docker-compose.yml`의 `graspnet` 서비스, `:8200`)에 HTTP로 보낸다. 원래는 추론 때마다 `docker run`으로 1회용 컨테이너를 띄웠는데, 호출마다 CUDA import + 체크포인트 로드를 다시 해서 추론 1회가 수십~수백 초가 됐고 `grasp` 노드에 Docker 소켓이 필요해 컨테이너화도 막혔다. 3단계(BR 4.1절) 비교 리포트는 `contact_graspnet`까지 붙은 뒤에나 3자 비교로 작성 가능하다.

**`graspnet_baseline`의 입력과 파지점 결정**: `grasp`는 `instance_masks`의 마스크 영역만 역투영한 **물체 단일 포인트클라우드**(camera frame)를 추론 서버에 보낸다(`node._candidates_for`). GraspNet 결과에서는 **자세(접근축·닫힘축)만 쓰고, 위치와 개폭은 같은 물체 클라우드로 실측해 다시 잡는다**(`graspnet_baseline._refine_on_cloud`, "되잡기") — 가로 위치는 물체 점의 중앙값, 깊이는 그 중심선 주변 국소 표면 + `refine_grasp_depth_mm`, 개폭은 손가락 패드가 지나는 창 안의 실측 폭이다. 물체만 잘라 넣는 것은 장면 전체를 넘기면 GPU 사용량이 커지기 때문이다. 대신 GraspNet은 장면 전체 클라우드로 학습된 모델이라 이 입력에서는 위치·폭 예측이 맞지 않아, 자세만 쓰도록 나눴다. 2026-09-07 실측에서 후보 위치는 물체에서 8~34mm 떨어진 허공이었고 폭도 크게 틀렸지만(네일: 실측 4.3mm vs 예측 57.1mm), 국소 형상에서 나오는 자세는 쓸 만했다. 되잡은 뒤에는 수직 대비 기울기 상한(`approach_angle_hard_max_deg`), 물릴 재료 유무(`no_grip_material`), 되잡은 폭의 그리퍼 한계(`width_invalid`)로 거르고, 최종 선택은 `control/grasp_selection.py`가 한다. **알려진 대가**: GraspNet 폭 출력이 100mm로 포화되고 Top-K 후보가 실제로는 1~2곳으로 뭉친다. 입력을 장면 전체 클라우드로 바꾸면 풀리는 문제지만(`graspnet_baseline.py` `_refine_on_cloud` docstring), GPU 부담 때문에 물체 단일 클라우드를 유지한다(2026-09-10 기준).

### 3.3 자동 라벨링 파이프라인 (`tools/training/auto_labeling/`)
...
이와 별개로, **운영 중에 쌓이는 라벨**이 하나 더 있다. `detector=vlm_sam`으로 돈 관측은 이미지와 VLM이 붙인 라벨을 `dataset_items`(+ `data/datasets/`)에 그대로 남기고, 데이터셋 화면에서 사람이 검토(`reviewed`)해 YOLO 학습셋으로 내보낸다(`GET /api/datasets/export`). 위 1~5번이 "학습 전에 만드는" 라벨이라면 이쪽은 "쓰면서 모이는" 라벨이다.
...
| 모델 가중치 관리 | 로컬 볼륨 마운트(`models/`, git 밖). GraspNet 체크포인트는 저장소 밖 assets(`PIECE_PICKING_ASSETS_DIR`) |
| 라벨링 | SAM+VLM 자동 라벨 + 데이터셋 화면에서 사람 검토 (3.3절) |
| 캘리브레이션 | OpenCV `calibrateHandEye` — `tools/calibration/` |
...
| 그리퍼 | OnRobot RG2 | 2지 평행 그리퍼. `/onrobot/sendCommand`로 개폭·힘을 주고, `/onrobot/status`의 `gsta` 비트(Busy / Grip detected)로 완료와 파지 성공을 판정한다. 컨트롤러 TCP 이름은 `GripperDA_v1` |
| 비전 센서 | RealSense RGB-D | 손목 장착(eye-in-hand). `realsense2_camera`가 `/camera/color/image_raw`·`/camera/aligned_depth_to_color/image_raw`·`/camera/color/camera_info`를 발행 — 이 토픽들은 외부 의존성이라 `인터페이스_정의서.md`에서 정의 대상이 아니다. **`align_depth`는 필수다**(depth가 color 픽셀 grid에 정렬돼 있어야 grasp가 포인트클라우드를 재구성한다) |
...
| 마이크 (2차, 6단계) | 로봇 PC 연결 USB 마이크 + 브라우저 마이크 | 웨이크워드("hello rokey")는 로봇 PC 마이크를 `tools/voice/wakeword_bridge.py`가 듣고, 명령 녹음·STT는 브라우저 마이크가 맡는다 (3.1b절) |
...
빌드 컨텍스트는 **저장소 루트**이고, `dockerfile:`로 각 패키지의 Dockerfile을 가리킨다. `sort_msgs`·`perception_common`처럼 패키지 폴더 밖에 있는 의존을 같은 이미지 안에서 빌드해야 하기 때문이다. 정본은 `docker-compose.yml`이며 아래는 구조만 요약한 것이다.

| 서비스 | 네트워크 | GPU | 요점 |
|---|---|---|---|
| `db` | bridge (`5432` 게시) | — | host 네트워크 컨테이너가 서비스 이름 DNS를 못 써서 포트를 게시한다 |
| `planner` | bridge (`8100` 게시) | — | 기동 시 `database/migrations/*.sql`을 적용한다. 스키마·시드(`objects.yaml`/`bins.yaml`)는 이미지에 굽지 않고 마운트 |
| `perception` | host | ✅ | `PERCEPTION_DETECTOR`로 `yolo`/`vlm_sam` 선택. 모델(`models/`)과 `data/`를 마운트 |
| `graspnet` | bridge (`8200` 게시) | ✅ | GraspNet-baseline 상주 추론 서버. 체크포인트는 저장소 밖 assets에서 마운트. `/health`는 모델을 다 올린 뒤에 열린다 |
| `grasp` | host | ✅ | `GRASP_STRATEGY`로 전략 선택(기본 `graspnet_baseline`). `grasp_params.yaml`을 마운트해 재빌드 없이 튜닝 |
| `control` | host | — | `FAKE_ROBOT`(기본 1)로 모의/실물 전환. `skill_params.yaml`·`bins.yaml`을 마운트 |
| `web` | bridge (`8000` 게시) | — | `MOCK_MODE=1` — `data/mock` 픽스처로 도는 개발용 |
| `web_ros` | host (`profile: ros`) | — | 같은 이미지를 `MOCK_MODE=0`으로 띄운 것. **`web`과 8000이 겹치므로 동시에 띄우지 않는다** |

세 가지 원칙이 이 구성을 지배한다.

- **DDS를 봐야 하는 것은 host 네트워크에 둔다.** 디스커버리가 멀티캐스트에 기대므로 브리지 네트워크에서는 호스트의 로봇 드라이버·카메라 노드를 찾지 못한다. 대신 host에는 서비스 이름 DNS가 없어 `planner`/`db`/`graspnet`을 게시된 포트로 `localhost` 호출한다.
- **튜닝 대상 설정은 이미지에 굽지 않는다.** `grasp_params.yaml`·`skill_params.yaml`·`bins.yaml`은 마운트라 `docker compose restart`만으로 반영된다. 실물 앞에서 값을 좁혀 나가는 데 이미지 재빌드가 끼면 사실상 불가능해진다.
- **쓰기가 필요한 경로만 골라서 rw로 연다.** `data/`는 읽기 전용이고 `data/unknown_crops`·`data/observations`·`data/datasets`·`data/point_clouds`만 별도로 rw다 — 캘리브레이션 결과 같은 입력을 실수로 덮어쓰지 않게 한다.

### 5.3 저장소 구성

| 경로 | 종류 | 역할 |
|---|---|---|
| `src/sort_msgs/` | ROS2 패키지 (ament_cmake) | 인터페이스 정의(`msg`/`action`) — 상세는 `인터페이스_정의서.md` |
| `src/perception/`, `src/grasp/`, `src/control/`, `src/web/`, `src/perception_common/` | ROS2 패키지 (ament_python) | 3.1절 참조 |
| `services/planner/` | 순수 Python (`COLCON_IGNORE`) | 계획 API. ROS2 빌드 대상 아님 |
| `web/frontend/` | TypeScript (Vite/React) | 화면 3종 — 상세는 `화면정의서.md` |
| `database/migrations/` | SQL | 2.2절 스키마. `planner`가 기동 시 순서대로 적용 |
| `tools/` | 순수 Python (`calibration`·`training`·`scripts`에 `COLCON_IGNORE`) | 캘리브레이션·학습·진단 스크립트, 웨이크워드 브리지 |
| `data/`, `models/` | 데이터 (`COLCON_IGNORE`) | 캘리브레이션 결과·mock 픽스처·관측/데이터셋 산출물, 모델 가중치 |
| `docs/` | 문서 | BR, 본 명세서, 인터페이스·화면 정의서, 문제 분석, 실험 리포트 |
...
| FR-07 (최소 두 가지 파지 전략) | 3.2절 — `heuristic_pca`·`graspnet_baseline` 2전략 구현으로 충족 |
